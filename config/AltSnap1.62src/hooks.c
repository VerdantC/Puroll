...
// TODO: Event Hook
    ...
    case WM_TIMER: {
        ...
        int rightoffset = CapButtonWidth+PinW+bdx+GetSystemMetricsForDpi(SM_CXBORDER, dpi)*2 - bdx*2.4;
        int topoffset = bdy+1-bdy*1.7;
...
static HWND CreatePinWindow(const HWND owner)
{
    ...
    if(!GetClassInfoEx(hinstDLL, TEXT(APP_NAMEA)TEXT("-Pin"), &wnd)) {
        ...
        wnd.hbrBackground = CreateSolidBrush(readColorFromFile("config/color_file.txt"));
